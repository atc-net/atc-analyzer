# 🔗 ATC210: Use expression body syntax when appropriate
...
7. **Property accessor exception**: The line length check is **not applied** to property accessor expression bodies (`get =>` and `set =>`). Property accessors are constrained by the property structure, and forcing a newline for the arrow may reduce readability
...
### Long property accessor expression bodies

Property accessor expression bodies (`get =>` and `set =>`) are **excluded** from the line length check. This allows compact property definitions even when the expression is long:

```csharp
// compliant - long accessor expression bodies are allowed (no line length enforcement)
public bool ShowOverlayTitle
{
    get => Camera.Overrides?.ShowOverlayTitle ?? settingsService.CameraDisplay.ShowOverlayTitle;
    set => Camera.Overrides!.ShowOverlayTitle = value;
}

// compliant - even very long expressions in accessors don't trigger the line length warning
public string Value
{
    get => value;
...
## 🔀 Overlap with Roslyn IDE Rules

This rule has functional overlap with the built-in Roslyn IDE expression body rules:

| IDE Rule | Description | Comparison |
|----------|-------------|------------|
| [IDE0021](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0021) | Use expression body for constructors | Both have code fixes |
| [IDE0022](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0022) | Use expression body for methods | Both have code fixes |
| [IDE0023/IDE0024](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0023-ide0024) | Use expression body for operators | Both have code fixes |
| [IDE0025](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0025) | Use expression body for properties | Both have code fixes |
| [IDE0026](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0026) | Use expression body for indexers | Both have code fixes |
| [IDE0027](https://learn.microsoft.com/en-us/dotnet/fundamentals/code-analysis/style-rules/ide0027) | Use expression body for accessors | Both have code fixes |

**Key advantage of ATC210:** This rule provides **configurable max line length** via `.editorconfig` and enforces proper arrow placement for long expressions. The IDE rules focus on whether to use expression bodies, while ATC210 also ensures proper formatting when expression bodies exceed the line length threshold.

### Configuration recommendation

If you enable ATC210, consider disabling the overlapping IDE rules to avoid duplicate diagnostics:

```editorconfig
[*.cs]
# Disable IDE expression body rules that overlap with ATC210
...
# Enable ATC210 with custom line length
dotnet_diagnostic.ATC210.severity = warning
dotnet_diagnostic.ATC210.max_line_length = 120
```
