# 🔗 ATC203: Method chains with 2 or more calls should be broken onto separate lines
...
9. **FluentAssertions Exception**: Any pattern starting with `.Should()` followed by exactly one assertion method (e.g., `actual.Should().Be(...)`, `actual.Should().NotBeNull()`, `actual.Should().BeEquivalentTo(...)`) is allowed on one line for test readability
   - Chains with 3+ methods (e.g., `.Should().Be(...).And.NotBeNull()`) must still be broken onto separate lines

## 💡 Motivation

Breaking method chains onto separate lines:
...
This rule is not configurable. Method chains with 2 or more method calls starting from a variable, property, or field must be formatted with each call on a separate line.
...
A code fix is available that automatically breaks method chains onto separate lines with proper indentation.

**How to apply:**
1. Position cursor on the method chain that violates the rule
2. Click the lightbulb (💡) or press `Ctrl+.` (Windows/Linux) or `Cmd+.` (Mac)
3. Select **"Break method chain onto separate lines"**

The code fix will:
- Break each method call in the chain onto its own line
...
- [ATC202: Multi parameters should be separated onto individual lines](ATC202.md) - Parameter formatting for multiple parameters
