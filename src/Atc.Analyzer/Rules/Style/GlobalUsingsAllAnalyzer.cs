...
public sealed class GlobalUsingsAllAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeUsingDirective(SyntaxNodeAnalysisContext context)
    {
        ...
        // Check if common namespaces should be excluded (deferred to ATC221)
        var options = context.Options.AnalyzerConfigOptionsProvider.GetOptions(context.Node.SyntaxTree);
        var excludeCommon = options.GetExcludeCommonNamespaces(RuleIdentifierConstants.Style.GlobalUsingsAll);

        if (excludeCommon)
        {
            var commonPrefixes = options.GetNamespacePrefixes(RuleIdentifierConstants.Style.GlobalUsingsCommon);
            if (commonPrefixes.Any(prefix => namespaceName!.StartsWith(prefix, StringComparison.Ordinal)))
            {
                return;
            }
        }
